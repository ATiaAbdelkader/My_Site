...
const publications = [
  { title: 'Évaluation des paramètres de qualité du sperme chez les béliers en conditions environnementales arides', journal: 'Journal of Animal Science', year: 2025, doi: '10.xxxx/jas.2025.001' },
  { title: 'Impact de la supplémentation nutritionnelle sur les performances reproductives des races bovines locales du sud algérien', journal: 'Livestock Science', year: 2024, doi: '10.xxxx/livsci.2024.002' },
  { title: 'Application de l\'analyse d\'images numériques pour l\'évaluation automatisée de la morphologie du sperme', journal: 'Theriogenology', year: 2024, doi: '10.xxxx/therio.2024.003' },
  { title: 'Étude comparative des systèmes d\'élevage traditionnels et modernes dans les régions arides d\'Algérie', journal: 'Animal Production Science', year: 2023, doi: '10.xxxx/aps.2023.004' },
  { title: 'Larves de la mouche soldat noire comme source alternative de protéines dans l\'alimentation des volailles : une revue', journal: 'Journal of Insects as Food and Feed', year: 2025, doi: '10.xxxx/jiff.2025.005' },
  { title: 'Effet du stress thermique sur les paramètres reproductifs chez les béliers : une méta-analyse', journal: 'Small Ruminant Research', year: 2024, doi: '10.xxxx/srr.2024.006' },
  { title: 'Outils d\'agriculture de précision pour la gestion du bétail en environnements arides', journal: 'Computers and Electronics in Agriculture', year: 2025, doi: '10.xxxx/comag.2025.007' },
  { title: 'Systèmes hydroponiques et aquaponiques pour une production alimentaire durable dans les régions sahariennes', journal: 'Agricultural Systems', year: 2023, doi: '10.xxxx/agsy.2023.008' },
];
  ...
  thoughts.forEach((t, i) => {
    ...
    card.innerHTML = `
      ...
      <a href="${t.link}" target="_blank" rel="noopener noreferrer" class="text-cyan-400 text-xs font-semibold flex items-center gap-1 hover:text-cyan-300 transition-colors">
        Lire la suite <i data-lucide="external-link" style="width:12px;height:12px"></i>
      </a>
    `;
    grid.appendChild(card);
  });
    ...
    card.innerHTML = `
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1 min-w-0">
          ...
          <button onclick="copyCitation('${p.title}', '${p.journal}', ${p.year}, '${p.doi}')" class="inline-flex items-center gap-1 text-amber-400 text-xs font-medium hover:text-amber-300 transition-colors mt-1"><i data-lucide="quote" style="width:12px;height:12px"></i> Copier la citation</button>
        </div>
  ...
  container.innerHTML = `
    <div class="glass-card rounded-2xl p-6 mb-8 overflow-hidden">
      <div class="text-center mb-4">
        <h3 class="text-white font-bold text-lg">🗺️ Carte des conférences</h3>
        <p class="text-gray-500 text-xs">Empreinte géographique des interventions internationales</p>
      </div>
      <div class="relative" style="max-width:600px;margin:0 auto;aspect-ratio:1.5;">
        ...
        <div style="position:absolute;top:42%;left:48%;"><div class="w-3 h-3 rounded-full bg-cyan-400 shadow-lg" style="box-shadow:0 0 10px #22d3ee;"></div><span class="text-[8px] text-cyan-400 absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap">Tunis</span></div>
        <div style="position:absolute;top:38%;left:45%;"><div class="w-3 h-3 rounded-full bg-amber-400 shadow-lg" style="box-shadow:0 0 10px #fbbf24;"></div><span class="text-[8px] text-amber-400 absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap">Alger</span></div>
        <div style="position:absolute;top:35%;left:40%;"><div class="w-3 h-3 rounded-full bg-cyan-400 shadow-lg" style="box-shadow:0 0 10px #22d3ee;"></div><span class="text-[8px] text-cyan-400 absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap">Marrakech</span></div>
        <div style="position:absolute;top:44%;left:44%;"><div class="w-3 h-3 rounded-full bg-amber-400 shadow-lg" style="box-shadow:0 0 10px #fbbf24;"></div><span class="text-[8px] text-amber-400 absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap">Ouargla</span></div>
        <div style="position:absolute;top:42%;left:46%;"><div class="w-3 h-3 rounded-full bg-cyan-400 shadow-lg" style="box-shadow:0 0 10px #22d3ee;"></div><span class="text-[8px] text-cyan-400 absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap">Biskra</span></div>
        <div style="position:absolute;top:43%;left:47%;"><div class="w-3 h-3 rounded-full bg-amber-400 shadow-lg" style="box-shadow:0 0 10px #fbbf24;"></div><span class="text-[8px] text-amber-400 absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap">El Oued</span></div>
      </div>
...
function copyCitation(title, journal, year, doi) {
  ...
  navigator.clipboard.writeText(apa).then(() => {
    showToast('📋 Citation APA copiée !', 'success');
  }).catch(() => {
    showToast('📋 ' + apa, 'info');
  });
}
...
function buildSearchIndex() {
  ...
  const skillNames = ['Analyse de Données', 'Conception Expérimentale', 'Rédaction Scientifique', 'R / Excel / Power BI', 'Développement d\'Applications', 'Visualisation de Données', 'Production Animale', 'Sélection & Amélioration', 'Nutrition Animale', 'Formateur FBS', 'Conception de Programmes', 'Communication'];
